describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let text = await button.getProperty("innerText");
      let text_value = await text["_remoteObject"].value;
      expect(text_value).toBe("Remove from Cart");
    }
    let text = await page.$("#cart-count");
    let text_value = await text.getProperty("innerHTML");
    let count = text_value["_remoteObject"].value;
